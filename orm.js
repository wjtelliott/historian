...
class ORM {
    ...
    const getMassiveWordCount = async (user, type, wordArg) => {
      ...
      response.rows.forEach((row) => {
        row.message_content.split(' ').forEach((word) => {
          if (type === 'length') if (!wordArg || word.length < wordArg) return;
          if (type === 'word')
            if (!wordArg || word.toLowerCase() !== wordArg.toLowerCase())
              return;
      ...
      if (type !== 'length')
        for (const word in dict) {
          if (type === 'amount') if (dict[word] != wordArg) delete dict[word];
          if (type === 'amount-lt')
  ...
  userData() {
    ...
    const queryUser = async (name) => {
      if (!name || name.trim() == '' || name.trim().length < 5) return null;
      const response = await client.query(
        `SELECT
