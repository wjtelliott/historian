...
class CWhoIs extends Command {
  constructor(logger, bot, orm) {
    async function run(target, args) {
      ...
      const response = await this.orm.userData().getUserData(userNameSearch);
      ...
      response.forEach(async (user) => {
        ...
        loggedMessages = isNaN(loggedMessages) ? 'Unknown' : loggedMessages;
...
class CFindWord extends Command {
  constructor(logger, bot, orm) {
    async function run(target, args) {
      ...
      if (typeof response === 'string') return await target.reply(response);
      const dict = {};
      let wordCount = 0;
      const re = new RegExp(`${word}`, 'gi');
      response.forEach((row) => {
        wordCount += row.message_content.match(re).length;
        if (!(row.user_discord_id in dict)) dict[row.user_discord_id] = 1;
